...
//
// Build:
//   gcc -O2 -o rover_monitor_12 rover_monitor_12.c ina260.c -lgpiod
//
...
#if 0
sudo apt-get update
sudo apt-get install -y build-essential libgpiod-dev i2c-tools

# Enable I2C if not already
sudo raspi-config nonint do_i2c 0 2>/dev/null || true

# Build
gcc -O2 -o pi_oled_shutdown_monitor pi_oled_shutdown_monitor.c -lgpiod

# Install binary
sudo install -m 0755 ./pi_oled_shutdown_monitor /usr/local/bin/pi_oled_shutdown_monitor
#endif
...
static int i2c_open_dev(const char *dev, int addr) {
...
static int ssd1306_cmd2(uint8_t c1, uint8_t c2) {
    if (ssd1306_cmd(c1) < 0) return -1;
    if (ssd1306_cmd(c2) < 0) return -1;
    return 0;
}
...
static int ssd1306_data(const uint8_t *data, size_t len) {
    ...
    while (len > 0) {
        ...
        if (write(i2c_fd, chunk, n+1) != (ssize_t)(n+1)) return -1;
        data += n;
        len  -= n;
    }
    return 0;
...
static int ssd1306_init(void) {
    ...
    if (ssd1306_cmd2(0xD5, 0x80) < 0) return -1;        // clock divide
    if (ssd1306_cmd2(0xA8, 0x3F) < 0) return -1;        // multiplex
    if (ssd1306_cmd2(0xD3, 0x00) < 0) return -1;        // display offset
...
static void ssd1306_set_pixel(int x, int y, bool on) {
    ...
    uint8_t bit = 1 << (y & 7);
    if (on) oled_buf[idx] |= bit; else oled_buf[idx] &= ~bit;
}

static void ssd1306_hline(int x, int y, int w, bool on) {
    for (int i = 0; i < w; ++i) ssd1306_set_pixel(x+i, y, on);
}

static int ssd1306_update(void) {
    if (ssd1306_cmd2(0x21, 0x00) < 0) return -1; // set column addr: start 0
    if (ssd1306_cmd(0x7F) < 0) return -1;        // end 127
    if (ssd1306_cmd2(0x22, 0x00) < 0) return -1; // set page addr: start 0
    if (ssd1306_cmd(0x07) < 0) return -1;        // end 7
...
static int glyph_width(uint8_t glyph[5]) {
    int left = 0, right = 4;
    while (left < 5) {
        if (glyph[left]) break;
        left++;
...
static void draw_char_prop(int x, int y, char c) {
    if (c < 32 || c > 127) c = '?';
    const uint8_t *g = font5x7[c - 32];
    // Determine visible columns
    int left = 0, right = 4;
    while (left < 5 && g[left] == 0) left++;
    while (right >= 0 && g[right] == 0) right--;
    if (right < left) { right = left; } // at least 1 col
    int w = right - left + 1;
...
static int draw_text_prop(int x, int y, const char *s) {
    int cursor = x;
    for (; *s; ++s) {
        char c = *s;
        if (c == '\n') { y += 10; cursor = x; continue; }
        if (c < 32 || c > 127) c = '?';
        uint8_t glyph[5];
        ...
        if (cursor >= SSD1306_WIDTH) break;
    }
    return cursor;
}

// ======== System info helpers ========
static int get_ip_address(char *out, size_t outlen) {
...
static int get_wifi_ssid(char *out, size_t outlen) {
...
static int get_cpu_temp_c(double *outC) {
    FILE *f = fopen("/sys/class/thermal/thermal_zone0/temp", "r");
    if (!f) return -1;
    long t = 0;
    if (fscanf(f, "%ld", &t) != 1) { fclose(f); return -1; }
    fclose(f);
    *outC = t / 1000.0;
    return 0;
}

static void fmt_uptime(char *out, size_t outlen) {
    // Read from /proc/uptime
    FILE *f = fopen("/proc/uptime", "r");
    double up = 0.0; 
    if (f) { 
        int i = fscanf(f, "%lf", &up); fclose(f); 
    }
...
static void get_ina260_status(float *voltage_mv, float *current_ma)
{
...
static int gpio_init(void) {
    ...
    if (!btn_line) { perror("get_line(button)"); return -1; }
    ...
    if (!rs_btn_line) { perror("get_line(rs_button)"); return -1; }
    ...
    if (!green_led_line) { perror("get_green_line(led)"); return -1; }
    ...
    if (!red_led_line) { perror("get_red_line(led)"); return -1; }
    ...
    if (!bell_line) { perror("get_bell_line(led)"); return -1; }
...
void gpio_set_green_led(int val) {
    if (green_led_line) gpiod_line_set_value(green_led_line, val ? 1 : 0);
}

void gpio_set_red_led(int val) {
    if (red_led_line) gpiod_line_set_value(red_led_line, val ? 1 : 0);
}

void gpio_set_bell(int val) {
    if (bell_line) gpiod_line_set_value(bell_line, val ? 1 : 0);
}

static void gpio_cleanup(void) {
    if (green_led_line) { gpiod_line_release(green_led_line); green_led_line = NULL; }
    if (red_led_line) { gpiod_line_release(red_led_line); red_led_line = NULL; }
    if (bell_line) { gpiod_line_release(bell_line); bell_line = NULL; }
    if (btn_line) { gpiod_line_release(btn_line); btn_line = NULL; }
    if (rs_btn_line) { gpiod_line_release(rs_btn_line); rs_btn_line = NULL; }
...
// ======== UI helpers ========
static void draw_status_screen(const char *ip, const char *ssid, double tempC, const char *uptime, double voltage_mv, double current_ma) {
    ssd1306_clear();
    int y = 0;

    draw_text_prop(0, y, "IP: "); draw_text_prop(24, y, ip && *ip ? ip : "—"); y += 12;
    draw_text_prop(0, y, "SSID: "); draw_text_prop(34, y, ssid && *ssid ? ssid : "—"); y += 12;

    // char tbuf[32]; snprintf(tbuf, sizeof tbuf, "CPU: %.1f\xC2\xB0""C", tempC);
    char tbuf[32]; snprintf(tbuf, sizeof tbuf, "CPU: %.1f ""C", tempC);

    draw_text_prop(0, y, tbuf); y += 12;
...
static void draw_message_center(const char *msg) {
    ssd1306_clear();
...
void* background_sound_thread(void* arg) {
    while (1) {
...
int main(void) {
    ...
    if (get_wifi_ssid(ssid, sizeof ssid) != 0) strncpy(ssid, "—", sizeof ssid);
            ...
            if (gpiod_line_event_read(rs_btn_line, &ev) == 0) {
                if (ev.event_type == GPIOD_LINE_EVENT_FALLING_EDGE) {
                    ...
                    if (rover_run_state != 0) {
                       printf("Stop Rover\n");
                       rover_run_state = 0;
//                       system("/home/jerryo/shut_down_button/script_off.sh");
//                       system("/home/jerryo/stop_rover.sh");
                       printf("'Off' script finished.\n");
                       gpio_set_green_led(0);
                    } else {
                       printf("Run Rover\n");
                       rover_run_state = 1;
//                       system("/home/jerryo/shut_down_button/script_on.sh");
//                       system("/home/jerryo/start_rover.sh");
                       printf("'On' script finished.\n");
                       gpio_set_green_led(1);
                    }
...
//        bool changed = false;
        ...
        if (get_ip_address(ip, sizeof ip) == 0) {
            if (strcmp(ip, last_ip) != 0) { changed = true; strncpy(last_ip, ip, sizeof last_ip); }
        } else {
            if (strcmp(last_ip, "—") != 0) { strncpy(last_ip, "—", sizeof last_ip); changed = true; }
        }

        if (get_wifi_ssid(ssid, sizeof ssid) == 0) {
            if (strcmp(ssid, last_ssid) != 0) { changed = true; strncpy(last_ssid, ssid, sizeof last_ssid); }
        } else {
            if (strcmp(last_ssid, "—") != 0) { strncpy(last_ssid, "—", sizeof last_ssid); changed = true; }
        }

        if (get_cpu_temp_c(&tempC) == 0) {
            // Update if temp changed by >= 0.5 C
            if (fabs(tempC - last_tempC) >= 0.5) { changed = true; last_tempC = tempC; }
        }
...
#define CURRENT_HIGH_LIMIT  (5000.0)  // 5 amps
        ...
        if (changed) {
            ...
            if (counter == 0) draw_status_screen(last_ip, last_ssid, last_tempC, upbuf,1200.0,500.0);
        }
        tick_cntr++;
    }
